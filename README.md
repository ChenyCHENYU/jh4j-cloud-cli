...
当前版本：`@agile-team/jh4j-cloud-cli@0.5.1`。
...
| 极简交互 | 默认只选择项目类型并确认项目名，其他参数全部采用模板默认值 |
| 默认可改 | 项目名直接回车使用默认值，也可以现场输入新名称 |
...
标准能力、标题、端口、联调地址、Registry、模块标识和各套环境配置都不再逐项询问。需要调整时，创建完成后直接修改项目中的 `project.config.json`、`.npmrc` 和 `.env*` 文件。

PC 项目名默认使用 `jh4j-ui-app`，移动端默认使用 `jh4j-mobile-app`。在项目名输入处直接回车即可接受，也可以输入新名称；还可以直接执行 `jh4j create my-app`。常见的人类可读名称会自动转换为适合目录和 `package.json` 的小写名称。
